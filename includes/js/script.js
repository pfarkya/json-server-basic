...
(function() {
        ...
        var response = $.ajax({
            ...
            success: function(data) {
                ...
                if (data[0]) {
                    ...
                    if ($('#addmore').length == 0) $('#main>.row>h2').after('\
                	<button class="btn btn-success pull-right"\
                  style="margin-top:20px;margin-bottom=10px;" id="addmore"\
             >Add More</button>');
                    if ($('#loadMore').length == 0) $('#main').append('<button class="btn btn-success"\
                  style="margin-top: 0px;margin-bottom: 20px;" id="loadMore"\
             >Load More</button>')
                    _start = data[data.length - 1]["id"] + 1;
                    updateCount();
                }
            }
        });
    ...
    $('#main').delegate('#loadMore', 'click', function() {
        $.ajax({
            url: "http://localhost:8080/addresses?_start=" + _start + "&_limit=" + _limit,
            dataType: "json",
            success: function(data) {
